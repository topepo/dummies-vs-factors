...
tempate_dummy <- 
  read.delim("ctree_dummy.R",
             sep = "\n", stringsAsFactors = FALSE, 
             colClasses = "character", quote = "",
             blank.lines.skip = FALSE, header = FALSE)[, 1]

tempate_factor <- 
  read.delim("ctree_factor.R",
             sep = "\n", stringsAsFactors = FALSE, 
             colClasses = "character", quote = "",
             blank.lines.skip = FALSE, header = FALSE)[, 1]

tempate_ordered <- 
  read.delim("ctree_ordered.R",
             sep = "\n", stringsAsFactors = FALSE, 
             colClasses = "character", quote = "",
             blank.lines.skip = FALSE, header = FALSE)[, 1]

tempate_ordereddummy <- 
  read.delim("ctree_ordereddummy.R",
             sep = "\n", stringsAsFactors = FALSE, 
             colClasses = "character", quote = "",
             blank.lines.skip = FALSE, header = FALSE)[, 1]
...
for(i in seeds) {
  ...
  cat(tmp_dummy, sep = "\n", 
      file =  file.path("..", "Code", 
                        paste0("ctree_dummy_", i, ".R")))
  
  tmp_factor <- tempate_factor
  tmp_factor <- gsub("SEED", i, tmp_factor)
  cat(tmp_factor, sep = "\n", 
      file =  file.path("..", "Code", 
                        paste0("ctree_factor_", i, ".R")))  
  
  tmp_ordered <- tempate_ordered
  tmp_ordered <- gsub("SEED", i, tmp_ordered)
  cat(tmp_ordered, sep = "\n", 
      file =  file.path("..", "Code", 
                        paste0("ctree_ordered_", i, ".R")))
  
  tmp_ordereddummy <- tempate_ordereddummy
  tmp_ordereddummy <- gsub("SEED", i, tmp_ordereddummy)
  cat(tmp_ordereddummy, sep = "\n", 
      file =  file.path("..", "Code", 
                        paste0("ctree_ordereddummy_", i, ".R")))  
}

###################################################################

r_names <-  c(paste0("ctree_dummy_", seeds, ".R"),
              paste0("ctree_factor_", seeds, ".R"),
              paste0("ctree_ordered_", seeds, ".R"),
              paste0("ctree_ordereddummy_", seeds, ".R"))
...
cat(paste("SHELL = /bin/bash\n",
          ...
          sep = ""),
    file = file.path("..", "Code", "ctree_make"))
